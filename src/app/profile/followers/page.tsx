...
export default async function FollowersPage() {
  ...
  const { data, error } = await supabase.from("follows").select("follower_id").eq("followee_id", user.id);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-emerald-50 via-white to-white pb-16">
      <div className="mx-auto max-w-5xl px-4 py-8 space-y-6">
        ...
        {error && <p className="text-sm text-red-600">フォロワー一覧の取得に失敗しました。</p>}
        {followers.length === 0 && !error && <p className="text-sm text-gray-600">まだフォロワーがいません。</p>}

        <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
          {followers.map((row) => {
            const fid = (row as { follower_id?: string }).follower_id ?? "";
