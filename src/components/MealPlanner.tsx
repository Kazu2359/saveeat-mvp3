  ...
  return (
    <section className="mt-10 rounded-2xl bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-2">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-emerald-600">AI献立サジェスト</p>
          <h2 className="text-xl font-bold text-gray-900">今ある食材から作りやすいメニュー</h2>
          <p className="mt-1 text-sm text-gray-600">
            在庫名をキーワード解析して、日本の家庭料理を1品提案します。足りない材料があればメモして買い足してください。
          </p>
        </div>
              ...
              {aiSuggestions.map((suggestion) => (
                ...
                >
                  <div className="flex flex-col gap-4">
                    <div>
                      <span className="text-lg font-semibold text-emerald-700">{suggestion.title}</span>
                      <p className="text-xs text-emerald-800">出典: {suggestion.source}</p>
                    </div>
                    ...
                    {suggestion.sides && suggestion.sides.length > 0 && (
                      <p className="text-xs text-gray-600">相性の良い副菜: {suggestion.sides.join(" / ")}</p>
                    )}
                  </div>
                </article>
              ))}
